// Vector redraw of the GNS 430 faceplate.
//
// Geometry is traced from the original 446x186 trainer bitmap (measured with a
// pixel grid), so this shares a coordinate space with bezel.js — the hit
// regions and the artwork cannot drift apart.
//
...
import { BEZEL, SCREEN } from './bezel.js';
...
export function bezelSvg() {
  ...
  const screenRecess = `
    <rect x="${SCREEN.x - 6}" y="${SCREEN.y - 8}" width="${SCREEN.w + 12}" height="${SCREEN.h + 16}"
          rx="4" fill="url(#recess)"/>
    <rect x="${SCREEN.x - 2}" y="${SCREEN.y - 2}" width="${SCREEN.w + 4}" height="${SCREEN.h + 4}"
          rx="1.5" fill="#000"/>`;
  ...
  const left = `
    ...
    ${key(
      ...
      28,
      32,
      `<text class="lbl" x="90" y="41" font-size="9" text-anchor="middle">C</text>
    ...
    ${key(
      ...
      28,
      32,
      `<text class="lbl" x="90" y="86" font-size="9" text-anchor="middle">V</text>
  ...
  const right = `
    ...
    ${key(
      ...
      73,
      23,
      `<path d="M377 23l10 0l-5 9z" fill="#e9e9e9"/>
    ...
    ${key(
      ...
      `<g stroke="#e9e9e9" stroke-width="1.5" fill="none">
         <path d="M375 58h18"/><path d="M383 52v12"/>
       </g>
       <ellipse cx="383" cy="58" rx="4.6" ry="6" fill="none" stroke="#e9e9e9" stroke-width="1.5"/>
       <path d="M391 54l6 4l-6 4z" fill="#e9e9e9"/>`
    )}
  ...
  return `<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" role="img"
       aria-label="Garmin GNS 430 faceplate">
  ${defs}
